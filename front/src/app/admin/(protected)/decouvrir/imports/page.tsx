import { PlaceImportsAdmin } from "@/components/admin/place-imports-admin";
...
export default async function PlaceImportsPage({
  searchParams,
}:{
  ...
  const [candidates,communes,categories]=await Promise.all([
    get<PlaceImportCandidate[]>(`/api/admin/place-imports/?status=${status}&limit=${PAGE_SIZE}&offset=${offset}`),
    get<Commune[]>("/api/communes/"),
    get<PlaceCategory[]>("/api/place-categories/"),
  ]);
  return <PlaceImportsAdmin
