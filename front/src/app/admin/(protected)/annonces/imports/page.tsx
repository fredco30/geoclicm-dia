import { ListingImportsAdmin } from "@/components/admin/listing-imports-admin";
...
export default async function ListingImportsPage({
  ...
  const [candidates, communes, categories] = await Promise.all([
    get<ListingImportCandidate[]>(
      `/api/admin/listing-imports/?status=${status}&limit=${PAGE_SIZE}&offset=${offset}`,
    ),
