...
async function fetchMyBusiness(slug: string): Promise<AdminBusinessDetail | null> {
  const cookieHeader = await getCookieHeader();
  const res = await fetch(
    `${API_URL}/api/advertiser/businesses/${slug}/`,
    {
  ...
  return (await res.json()) as AdminBusinessDetail;
}
...
export default async function EditMyBusinessPage({ params }: Props) {
  ...
  const [business, categories, communes] = await Promise.all([
    fetchMyBusiness(slug),
    fetchCategories(),
    fetchCommunes(),
  ]);
