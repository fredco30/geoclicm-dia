...
export function PlaceImportsAdmin({
  initialCandidates,
  communes,
  categories,
  filterStatus,
  ...
  initialCandidates: PlaceImportCandidate[];
  communes: Commune[];
  ...
  const act = (row: PlaceImportCandidate, action: "approve" | "reject") =>
    startTransition(async () => {
      const token = await ensureCsrf();
      const response = await apiFetch(
        `/api/admin/place-imports/${row.id}/${action}/`,
        {
          method: "POST",
      ...
      setMessage(
        action === "approve"
          ? `« ${row.title} » publié dans Découvrir.`
          : `« ${row.title} » rejeté.`,
      );
    });
  ...
  const bulkApprove = () =>
    startTransition(async () => {
      const token = await ensureCsrf();
      const response = await apiFetch(
        "/api/admin/place-imports/bulk-approve/",
        {
          method: "POST",
          headers: token ? { "X-CSRFToken": token } : {},
    ...
    <div>
        ...
        {filterStatus === "pending" && pendingRows.length > 0 ? (
          <div className="mt-4 flex flex-wrap items-center gap-2 rounded-xl border bg-white p-3 shadow-sm">
            ...
            <span className="text-xs text-slate-500">
              Les « fiables » ont commune + catégorie + preuve vérifiée. Décoche
              ceux à exclure avant d&apos;approuver.
            </span>
          </div>
        ) : null}
      </div>
              ...
              <div className="p-4">
                ...
                <div>
                  ...
                  <p className="mt-1 text-sm text-slate-600">
                    {row.address || row.commune_name || "Adresse manquante"}
                  </p>
                </div>
      ...
      <nav className="mt-6 flex items-center justify-between">
        ...
        <div className="flex gap-2">
          {page > 1 ? (
            <Link
              href={`/admin/decouvrir/imports?status=${filterStatus}&page=${page - 1}`}
