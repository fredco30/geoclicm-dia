...
function isReliable(row: BusinessImportCandidate) {
  return (
    row.commune != null &&
    row.category != null &&
...
type Group = "all" | "associations" | "commerces";

export function BusinessImportsAdmin({
  initialCandidates,
  communes,
  categories,
  filterStatus,
  ...
  initialCandidates: BusinessImportCandidate[];
  communes: Commune[];
  ...
  const associationCategoryIds = useMemo(() => {
    ...
    for (const c of categories) {
      if (c.parent != null && rootIds.has(c.parent)) ids.add(c.id);
    }
    return ids;
  }, [categories]);

  const isAssociation = (row: BusinessImportCandidate) =>
    row.category != null && associationCategoryIds.has(row.category);

  const visibleRows = useMemo(() => {
    if (group === "associations") return rows.filter(isAssociation);
    if (group === "commerces") return rows.filter((row) => !isAssociation(row));
    return rows;
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [rows, group, associationCategoryIds]);

  const counts = useMemo(
    () => ({
      all: rows.length,
      associations: rows.filter(isAssociation).length,
      commerces: rows.filter((row) => !isAssociation(row)).length,
    }),
    // eslint-disable-next-line react-hooks/exhaustive-deps
    [rows, associationCategoryIds],
  );
  ...
  const act = (row: BusinessImportCandidate, action: "approve" | "reject") =>
    startTransition(async () => {
      const token = await ensureCsrf();
      const response = await apiFetch(
        `/api/admin/business-imports/${row.id}/${action}/`,
        {
          method: "POST",
  ...
  const bulkApprove = () =>
    startTransition(async () => {
      const token = await ensureCsrf();
      const response = await apiFetch(
        "/api/admin/business-imports/bulk-approve/",
        {
          method: "POST",
          headers: token ? { "X-CSRFToken": token } : {},
      ...
      setRows((current) =>
        current.filter(
          (row) => !(selected.has(row.id) && row.status === "pending" && row.commune && row.category),
        ),
      );
      setSelected(new Set());
      setMessage(
    ...
    <div>
      <div className="mb-4">
        ...
        {filterStatus === "pending" ? (
          <nav className="mt-2 flex flex-wrap gap-2" aria-label="Filtrer par type">
            {(
              [
                ["all", `Tout (${counts.all})`],
                ["associations", `Associations (${counts.associations})`],
                ["commerces", `Commerces & services (${counts.commerces})`],
              ] as [Group, string][]
            ).map(([value, label]) => (
              <button
                key={value}
