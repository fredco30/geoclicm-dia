...
function schedule(row:EventImportCandidate){if(row.occurrences&&row.occurrences.length>0&&row.starts_at)return date(row.starts_at);const hint=row.extraction_evidence&&row.extraction_evidence.length?` « ${row.extraction_evidence[0]} »`:"";return `Récurrence ou période non datée — à préciser${hint}`;}

// Un évènement « fiable » a commune + catégorie + preuve vérifiée + une date
// de fin future (on n'auto-valide jamais un évènement passé ou non daté).
...
export function EventImportsAdmin({initialCandidates,communes,categories,filterStatus,page,hasNextPage}:{initialCandidates:EventImportCandidate[];communes:Commune[];categories:EventCategory[];filterStatus:string;page:number;hasNextPage:boolean}){
 ...
 {row.image_url?<img src={row.image_url} alt="" loading="lazy" decoding="async" className="h-full min-h-48 w-full object-cover" referrerPolicy="no-referrer"/>:<div className="flex min-h-48 items-center justify-center bg-slate-100 text-xs text-slate-400">Pas d&apos;image détectée</div>}<div className="p-4"><div className="flex items-center justify-between gap-2"><p className="text-xs font-medium uppercase text-[#a8533a]">{row.status==="expired"?"Expiré— consultation uniquement":`${row.source_label} · ${row.extraction_method==="ai"?"Extraction IA à vérifier":row.extraction_method==="ics"?"ICS":"JSON-LD"}`}</p><a href={row.source_url} target="_blank" rel="noopener noreferrer" className="inline-flex shrink-0 items-center gap-1 text-xs text-[#1a4d6e] underline">Source officielle <ExternalLink className="h-3 w-3"/></a></div><div><h2 className="font-serif text-xl font-semibold">{row.title}</h2><p className="mt-1 text-sm text-slate-600">{schedule(row)} · {row.venue_name||row.address||"Lieu manquant"}</p></div>{row.short_description?<p className="mt-3 line-clamp-3 text-sm text-slate-600">{row.short_description}</p>:null}{row.extraction_evidence.length?<blockquote className="mt-3 border-l-2 border-[#1a4d6e] pl-3 text-xs italic text-slate-600">Preuve détectée : « {row.extraction_evidence[0]} »</blockquote>:null}{row.validation_errors.length?<p className="mt-3 rounded bg-amber-50 p-2 text-xs text-amber-900">{row.validation_errors.join(" · ")}</p>:null}{row.status!=="expired"?<><div className="mt-4 flex flex-wrap items-center gap-2"><Select aria-label="Commune" value={row.commune??""} onChange={event=>update(row.id,"commune",event.target.value)} className="w-44"><option value="">Choisir la commune *</option>{communes.map(item=><option key={item.id} value={item.id}>{item.name}</option>)}</Select><Select aria-label="Catégorie" value={row.category??""} onChange={event=>update(row.id,"category",event.target.value)} className="w-44"><option value="">Choisir la catégorie *</option>{categories.map(item=><option key={item.id} value={item.id}>{item.name}</option>)}</Select><div className="ml-auto flex gap-2"><Button variant="secondary" disabled={pending} onClick={()=>act(row,"reject")}><X className="h-4 w-4"/> Rejeter</Button><Button disabled={pending||!row.commune||!row.category||!row.starts_at||!row.ends_at} onClick={()=>act(row,"approve")}><Check className="h-4 w-4"/> Approuver et publier</Button></div></div></>:null}</div></article>)}</div>}<nav className="mt-6 flex items-center justify-between"><span className="text-sm text-slate-500">Page {page}</span><div className="flex gap-2">{page>1?<Link href={`/admin/agenda/imports?status=${filterStatus}&page=${page-1}`} className="rounded border bg-white px-3 py-2 text-sm">← Précédente</Link>:null}{hasNextPage?<Link href={`/admin/agenda/imports?status=${filterStatus}&page=${page+1}`} className="rounded border bg-white px-3 py-2 text-sm">Suivante →</Link>:null}</div></nav></div>;
}
