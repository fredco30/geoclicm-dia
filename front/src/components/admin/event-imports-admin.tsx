...
export function EventImportsAdmin({initialCandidates,communes,categories,filterStatus,page,hasNextPage}:{initialCandidates:EventImportCandidate[];communes:Commune[];categories:EventCategory[];filterStatus:string;page:number;hasNextPage:boolean}){
 ...
 return <div><div className="mb-4"><Link href="/admin/agenda" className="text-sm text-slate-600">? Agenda</Link><h1 className="text-xl font-bold">Candidats Agenda</h1><p className="text-sm text-slate-500">Les évènements terminés avant le crawl sont conservés comme expirés, mais ne sont jamais proposés à la validation.</p><nav className="mt-3 flex flex-wrap gap-2">{[["pending","? valider"],["invalid","Incomplets"],["expired","Expirés"]].map(([value,label])=><Link key={value} href={`/admin/agenda/imports?status=${value}`} className={`rounded-full border px-3 py-1 text-xs ${filterStatus===value?"bg-[#1a4d6e] text-white":"bg-white text-slate-700"}`}>{label}</Link>)}</nav>
